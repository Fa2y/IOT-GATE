    ...
    def run(self):
        while True:
            ...
            for i in range(12):
                ...
                loop.run_until_complete(flow_stream_add(redis_keys.flows_stream_key(), now_timestamp, ','.join(connections)))
                print(f"[+] Adding flows to redis stream:{redis_keys.flows_stream_key()}:{','.join(connections)}:{now_timestamp}")
...
def main():
    ...
    while True:
        ...
        loop.run_until_complete(alert_stream_add(redis_keys.alerts_stream_key(), now_timestamp, attack_host, "Malicious activity detected, you should take immediate actions!"))
